# RDL Julia エンジン設計図 v2.0

*RDL概念的数理モデル v2.2 準拠 / 0から再設計*
...
Mが強い次元ほど揺らぎが小さい。
"""
function generate_xi(M, params::RDLParams; rng=Random.default_rng())
...
出力：
  ...
  ξ        揺らぎ
"""
function core_step(EFP_new, F_prev, M, H, params::RDLParams; rng=Random.default_rng())
    # EFPをMで解釈 → 作用解釈F
...
揺らぎξが根源的な自由度として関与する。
"""
function update_M(M, F, E, H, ξ, params::RDLParams)
    # Fの方向（どの次元で整合が起きているか）
...
```julia
"""
    rdl_step(EFP_new, F_prev, M, H, theta, params, neuro; rng)
...
"""
function rdl_step(EFP_new, F_prev, M, H, theta,
                  params::RDLParams, neuro::NeuroState;
...
function update() {
    ...
    // Julia呼び出し
    const result = rdlStep(EFP, F_prev, M, H, theta, params, neuro);
...
RDL Julia エンジンは、EFPをMで解釈しFを生成し、慣性投影との誤差EでMを更新する計算カーネルとして設計する。意味付けと判断はすべてp5.js側に委ねる。
