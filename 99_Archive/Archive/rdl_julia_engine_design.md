...
RDLエンジンを、意味判断から切り離した計算基盤として設計する。
...
`RDLParams` は研究者・開発者用の低次パラメータ群として扱う。
実運用や個体設計では、少数の上位ラベルを持つ `AgentProfile` から `RDLParams`、`NeuroState`、`theta` へ展開する。
...
function noise_time_scale(params::RDLParams)
    return sqrt(params.T)
end

function core_step(S, M, F, H, params::RDLParams; rng=Random.default_rng())
    E = F .- M .* S
    err = norm(E)
...
```julia
function heat_gate(H, params::RDLParams)
...
function update_M(M, S, E, H, params::RDLParams)
...
物理計算・熱・揺らぎ・構造更新の基礎設定は `RDLParams` にまとめる。
...
function neuro_mod(H, theta, neuro::NeuroState, params::RDLParams)
    theta_mod = theta
...
```julia
function rdl_step(S, M, F, H, theta, params::RDLParams, neuro::NeuroState; rng=Random.default_rng())
...
profile = AgentProfile(stability=0.8, curiosity=0.3, caution=0.7)
params, neuro, theta = make_profile(profile)
result = rdl_step(S, M, F, H, theta, params, neuro; rng=rng)
```
...
```javascript
const result = rdlStep(S, M, F, H, theta, params);
...
RDL Julia エンジンは、意味判断を外部に委ね、S・M・F・H・ξ の更新だけを行う物理計算カーネルとして設計する。
