# RDL 応用実装層:LangGraph / LangChain 借用 v0.1
*(RDL概念的数理モデル v2.3 の応用実装補完別冊)*
*(LLM エージェントオーケストレーション形式を局所線形近似として借用・RDL意味論の中で動かす)*
...
RDL_応用実装層_LangGraph_LangChain借用_v0.1(本文書)
...
**流用できない概念(RDL 固有・LangGraph/LangChain に存在しない)**
...
形式は RDL の動態を実装するための道具であり
RDL の哲学的基底(ξ + B + M_B)には従属しない
